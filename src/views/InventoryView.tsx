  ...
  return (
    <div className="space-y-6 lg:h-[calc(100vh-180px)] flex flex-col font-outfit animate-in fade-in slide-in-from-bottom-4 duration-700">
        ...
        <div className="flex flex-col sm:flex-row items-center gap-4 w-full lg:w-auto">
           <div className="relative group w-full sm:w-auto flex-1 lg:flex-none">
             ...
             <input
               ...
               className="pl-14 pr-6 h-14 md:h-16 bg-slate-200/50 dark:bg-white/[0.02] border border-slate-200 dark:border-white/5 rounded-2xl text-[10px] font-black text-foreground focus:border-cyan-500/30 outline-none w-full lg:w-80 uppercase tracking-widest"
             />
           </div>
           <button 
             onClick={() => setIsAdding(true)}
             className="h-14 md:h-16 w-full sm:w-auto px-10 bg-white text-black rounded-2xl font-black text-[10px] uppercase tracking-[0.2em] transition-all hover:scale-105 active:scale-95 flex items-center justify-center gap-3 shadow-2xl"
           >
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-3 md:gap-4 px-4">
          <div className="glass-card p-4 md:p-6 rounded-[1.5rem] md:rounded-[2rem] border-white/5">
            <p className="text-[8px] md:text-[9px] font-black text-slate-500 uppercase tracking-widest mb-1">Catálogo Total</p>
            <p className="text-lg md:text-xl font-black text-foreground">{products.length} Items</p>
          </div>
          <div className="glass-card p-4 md:p-6 rounded-[1.5rem] md:rounded-[2rem] border-white/5">
            <p className="text-[8px] md:text-[9px] font-black text-slate-500 uppercase tracking-widest mb-1">Valorización</p>
            <p className="text-lg md:text-xl font-black text-emerald-500">{formatCurrency(totalValue)}</p>
          </div>
          <div className="glass-card p-4 md:p-6 rounded-[1.5rem] md:rounded-[2rem] border-white/5">
            <p className="text-[8px] md:text-[9px] font-black text-slate-500 uppercase tracking-widest mb-1">Quiebre Stock</p>
            <p className={`text-lg md:text-xl font-black ${criticalStock > 0 ? 'text-amber-500' : 'text-foreground'}`}>{criticalStock} Alert</p>
          </div>
          <div className="glass-card p-4 md:p-6 rounded-[1.5rem] md:rounded-[2rem] border-white/5">
            <p className="text-[8px] md:text-[9px] font-black text-slate-500 uppercase tracking-widest mb-1">Stock Crítico</p>
            <p className={`text-lg md:text-xl font-black ${negativeStock > 0 ? 'text-rose-500 animate-pulse' : 'text-foreground'}`}>{negativeStock} Items</p>
          </div>
      </div>

      {/* Main Area */}
      <div className="px-4 pb-10 flex-1 overflow-y-auto no-scrollbar">
        <div className="glass-card rounded-[2rem] md:rounded-[3.5rem] overflow-hidden border-white/5 shadow-2xl relative">
          {/* Add Form Overlay */}
          {isAdding && (
            <div 
              ref={formRef}
              className="p-6 md:p-10 border-b border-white/5 bg-cyan-500/[0.02] space-y-6 md:space-y-10 animate-in slide-in-from-top-4 duration-500 relative overflow-hidden"
            >
               ...
               <div className="flex justify-between items-center relative z-10">
                  <div className="flex items-center gap-4">
                    ...
                    <h3 className="text-lg md:text-xl font-black text-foreground uppercase tracking-tighter">{editingId ? 'Editar Maestro' : 'Nuevo Registro'}</h3>
                  </div>
               ...
               <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-5 gap-6 md:gap-8 relative z-10">
                  <div className="sm:col-span-2 lg:col-span-2 space-y-3">
                    <label className="text-[9px] md:text-[10px] font-black text-slate-500 uppercase tracking-[0.3em] ml-2">Nombre del Producto</label>
                    <input 
                      type="text" 
                      value={name} 
                      onChange={e=>setName(e.target.value)} 
                      placeholder="DESCRIPCIÓN COMERCIAL..."
                      className="w-full bg-slate-200/50 dark:bg-white/[0.02] border border-slate-200 dark:border-white/5 rounded-2xl px-6 py-4 md:py-5 text-sm font-black text-foreground focus:border-cyan-500/30 outline-none transition-all uppercase tracking-widest"
                    />
                  </div>
                  <div className="space-y-3">
                    <label className="text-[9px] md:text-[10px] font-black text-slate-500 uppercase tracking-[0.3em] ml-2">SKU / Identificador</label>
                    <input 
                      type="text" 
                      value={sku} 
                  ...
                  <div className="space-y-3">
                    <label className="text-[9px] md:text-[10px] font-black text-slate-500 uppercase tracking-[0.3em] ml-2">Precio Neto</label>
                    <div className="relative group">
                       ...
                       <input 
                         ...
                         className="w-full bg-slate-200/50 dark:bg-white/[0.02] border border-slate-200 dark:border-white/5 rounded-2xl pl-14 pr-6 py-4 md:py-5 text-sm font-black text-foreground focus:border-cyan-500/30 outline-none transition-all"
                       />
                    </div>
                  </div>
                  <div className="space-y-3">
                    <label className="text-[9px] md:text-[10px] font-black text-slate-500 uppercase tracking-[0.3em] ml-2">Stock Inicial</label>
                    <input 
                      type="number" 
                      value={stock} 
                      onChange={e=>setStock(e.target.value)} 
                      className="w-full bg-slate-200/50 dark:bg-white/[0.02] border border-slate-200 dark:border-white/5 rounded-2xl px-6 py-4 md:py-5 text-sm font-black text-foreground focus:border-cyan-500/30 outline-none transition-all"
                    />
                  </div>
               </div>

               <div className="flex justify-end gap-4 relative z-10">
                  <button onClick={handleSave} className="w-full sm:w-auto px-10 py-5 bg-white text-black rounded-2xl font-black text-[10px] uppercase tracking-[0.2em] transition-all hover:scale-105 active:scale-95 flex items-center justify-center gap-3 shadow-lg">
              ...
              <div key={p.id} className="p-5 space-y-4">
                <div className="flex items-start justify-between gap-4">
                  <div className="flex items-start gap-4">
                    ...
                    <div className="space-y-0.5">
                      ...
                      <p className="text-[9px] font-black text-slate-400 uppercase tracking-widest">{p.sku || 'SIN SKU'}</p>
                    </div>
                  </div>
                  <div className="flex flex-col items-end gap-1">
                    <span className="text-sm font-black text-foreground tracking-tighter">{formatCurrency(p.net_price)}</span>
                ...
                <div className="flex gap-2 pt-2">
                  <button 
                    ...
                    className="flex-1 h-12 bg-slate-100 dark:bg-white/5 rounded-xl text-slate-600 dark:text-white flex items-center justify-center gap-2 text-[10px] font-black uppercase tracking-widest border border-slate-200 dark:border-white/10"
                  >
                    <Edit3 size={14} /> Editar
                  </button>
                  <button 
                    onClick={() => handleDelete(p.id)} 
                    className="w-12 h-12 bg-rose-500/10 rounded-xl text-rose-500 flex items-center justify-center border border-rose-500/20"
                  >
                    <Trash2 size={16} />
                  </button>
                </div>
                ...
                {filtered.map(p => (
                  ...
                  >
                    <td className="p-8 pl-10">
                      <div className="flex items-center gap-4">
                         <div className="w-10 h-10 rounded-xl bg-slate-100 dark:bg-white/5 flex items-center justify-center text-slate-400 group-hover:text-cyan-400 transition-colors"><Package size={16} /></div>
                         <p className="text-sm font-black text-foreground uppercase tracking-tighter">{p.name}</p>
                      </div>
                    </td>
                    <td className="p-8">
                      <span className="text-[10px] font-black px-3 py-1 bg-slate-100 dark:bg-white/5 rounded-lg text-slate-500 uppercase tracking-widest">{p.sku || 'N/A'}</span>
                    </td>
                    ...
                    <td className="p-8 pr-10 text-right">
                      <div className="flex justify-end gap-2 opacity-100 lg:opacity-0 lg:group-hover:opacity-100 transition-all transform lg:translate-x-4 lg:group-hover:translate-x-0">
                        <button onClick={() => handleEdit(p)} className="w-10 h-10 rounded-xl bg-slate-100 dark:bg-slate-900 flex items-center justify-center text-slate-500 dark:text-white hover:text-foreground transition-all border border-slate-200 dark:border-white/10 shadow-sm"><Edit3 size={14}/></button>
